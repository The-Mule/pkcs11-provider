...
if [ $# -ne 1 ]; then
    ...
    exit 1
elif [ -z "$TESTSSRCDIR" ]; then
    echo "Missing TESTSSRCDIR!"
    exit 1
fi
...
TMPPDIR="${TESTBLDDIR:-"/tmp"}/${TOKENTYPE}"
...
token_setup()
{
    ...
    if [ "${TOKENTYPE}" == "softhsm" ]; then
        ...
        SHARED_EXT=".so" SOFTOKNPATH="/usr/lib64" source "${TESTSSRCDIR}/softokn-init.sh"
...
cleanup() 
{
    ...
    title LINE "Result is $TEST_RESULT"
}
